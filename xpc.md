...
su -s /bin/bash thingsboard -c '
  ...
  /usr/share/thingsboard/bin/thingsboard.jar >> /var/log/thingsboard/thingsboard.log 2>&1 &
  echo $! > /var/run/thingsboard/thingsboard.pid
  echo "Started PID $!"
'
```

### Check ThingsBoard status
```bash
kill -0 $(cat /var/run/thingsboard/thingsboard.pid) 2>/dev/null && echo "running" || echo "stopped"
```
